...
const Form = ({ user }) => {
  ...
  //medyayi storage'e yukler ve url ini dondurur
  const uploadImage = async (image) => {
    if (!image) {
      return null;
    }
    // else if (
    //   //  foto tip png veya jpeg esitse calisir
  ...
  const handleSubmit = async (e) => {
    ...
    // todo fotografini urlini veritabanina ekle
    if (!textContent && !imageContent) {
      toast.info("tweet icerigi ekleyiniz");
      return;
    }
    setIsLoading(true);
    //medyayi  yukleme
  ...
  return (
    ...
    >
      ...
      <div className="w-full">
        <input
          className="w-full bg-transparent my-2 outline-none text-lg"
        ...
        <div className="flex justify-between">
          <label
            htmlFor="picture"
            className="hover:bg-gray-800 text-normal md:text-lg transition p-4 cursor-pointer rounded-full"
          >
            <BsCardImage />
